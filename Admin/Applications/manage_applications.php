...
<body>
    ...
    if (isset($_POST['update'])) {
        ...
        $dup_check_query = mysqli_query($link, "SELECT * FROM central.applications WHERE First_Name = '$first_name' AND Sur_Name = '$sur_name' AND Father_Name = '$father_name' AND DOB = '$dob' AND App_No != '$appNo'");
        if (mysqli_num_rows($dup_check_query) != 0 && $force_insert != "1") {
            $dup = mysqli_fetch_assoc($dup_check_query);
            echo "<script>
                    setTimeout(function() {
                        let confirmMsg = 
                            ...
                            'Father Name: {$dup['Father_Name']}\\n' +
                            'DOB: {$dup['DOB']}\\n\\n' +
                            'Do you want to update this application anyway?';
